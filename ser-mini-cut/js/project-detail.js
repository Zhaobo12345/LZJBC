...
const ProjectDetailPage = (function() {
    ...
    let noActivityActive = false;
    let noArchitectureActive = false;

    function toggleNoActivity() {
        noActivityActive = !noActivityActive;
        var navItem = document.getElementById('navNoActivity');
        var activityCard = document.getElementById('activityCard');

        if (noActivityActive) {
            if (navItem) navItem.classList.add('scene-active');
            if (activityCard) {
                activityCard.style.setProperty('display', 'none', 'important');
            }
        } else {
            if (navItem) navItem.classList.remove('scene-active');
            if (activityCard) {
                activityCard.style.removeProperty('display');
            }
        }
    }

    function toggleNoArchitecture() {
        noArchitectureActive = !noArchitectureActive;
        var navItem = document.getElementById('navNoArchitecture');
        var archNavItem = document.getElementById('archNavItem');
        var levelSelector = document.getElementById('levelSelector');
        var inviteNavItem = document.getElementById('inviteNavItem');
        var createContractBtn = document.getElementById('createContractBtn');
        var exitProjectWrap = document.getElementById('exitProjectWrap');

        if (noArchitectureActive) {
            if (navItem) navItem.classList.add('scene-active');
            if (archNavItem) archNavItem.style.setProperty('display', 'none', 'important');
            if (levelSelector) levelSelector.style.setProperty('display', 'none', 'important');
            if (inviteNavItem) inviteNavItem.style.setProperty('display', 'flex', 'important');
            if (createContractBtn) createContractBtn.style.setProperty('display', 'inline-flex', 'important');
            if (exitProjectWrap) exitProjectWrap.style.setProperty('display', 'flex', 'important');
        } else {
            if (navItem) navItem.classList.remove('scene-active');
            if (archNavItem) archNavItem.style.removeProperty('display');
            if (levelSelector) levelSelector.style.removeProperty('display');
            if (inviteNavItem) inviteNavItem.style.setProperty('display', 'none', 'important');
            if (createContractBtn) createContractBtn.style.setProperty('display', 'none', 'important');
            if (exitProjectWrap) exitProjectWrap.style.setProperty('display', 'none', 'important');
        }
    }

    var currentInviteTarget = '';
    var currentInviteMethod = 'phone';
    var currentContractTarget = '';
    ...
    function showAppModal(title, content, onConfirm, showCancel) {
        var modalTitle = document.getElementById('appModalTitle');
        var modalContent = document.getElementById('appModalContent');
        var modalCancel = document.getElementById('appModalCancel');
        var modalConfirm = document.getElementById('appModalConfirm');
        var modal = document.getElementById('appModal');
        if (!modalTitle || !modalContent || !modalCancel || !modalConfirm || !modal) return;

        modalTitle.textContent = title;
        modalContent.textContent = content;
        modalCancel.style.display = showCancel !== false ? 'flex' : 'none';
    ...
    function showInviteMemberModal(targetName) {
        currentInviteTarget = targetName;
        var targetEl = document.getElementById('inviteTargetName');
        var phoneEl = document.getElementById('invitePhone');
        if (targetEl) targetEl.textContent = targetName;
        if (phoneEl) phoneEl.value = '';
        var activeMethod = document.querySelector('.invite-method.active');
        if (activeMethod) selectInviteMethod(activeMethod, 'phone');
    ...
    function selectInviteMethod(element, method) {
        currentInviteMethod = method;
        document.querySelectorAll('.invite-method').forEach(function(m) { m.classList.remove('active'); });
        if (element) element.classList.add('active');

        var phoneForm = document.getElementById('phoneInviteForm');
        var linkForm = document.getElementById('linkInviteForm');
        var qrcodeForm = document.getElementById('qrcodeInviteForm');
        if (phoneForm) phoneForm.style.display = method === 'phone' ? 'block' : 'none';
        if (linkForm) linkForm.style.display = method === 'link' ? 'block' : 'none';
        if (qrcodeForm) qrcodeForm.style.display = method === 'qrcode' ? 'block' : 'none';
    }
    ...
    function sendInvite() {
        if (currentInviteMethod === 'phone') {
            var phone = document.getElementById('invitePhone');
            var phoneVal = phone ? phone.value : '';
            if (!phoneVal.trim()) {
                showAppModal('提示', '请输入手机号码', null, false);
                return;
            }
            if (!/^1[3-9]\d{9}$/.test(phoneVal)) {
                showAppModal('提示', '请输入正确的手机号码', null, false);
                return;
            }
            showAppModal('发送确认', '确定要邀请 ' + phoneVal + ' 加入"' + currentInviteTarget + '"吗？', function() {
                showToast('邀请已发送');
                closeInviteMemberModal();
            });
        } else if (currentInviteMethod === 'link') {
            showToast('链接已复制到剪贴板');
            closeInviteMemberModal();
        } else {
            showToast('二维码已生成');
    ...
    function showCreateContractModal(targetName) {
        currentContractTarget = targetName;
        var targetEl = document.getElementById('contractTargetName');
        if (targetEl) targetEl.textContent = targetName;

        var typeEl = document.getElementById('contractType');
        var nameEl = document.getElementById('contractName');
        var partyAEl = document.getElementById('contractPartyA');
        var partyBEl = document.getElementById('contractPartyB');
        var amountEl = document.getElementById('contractAmount');
        if (typeEl) typeEl.selectedIndex = 0;
        if (nameEl) nameEl.value = '';
        if (partyAEl) partyAEl.selectedIndex = 0;
        if (partyBEl) partyBEl.selectedIndex = 0;
    ...
    function createContract() {
        ...
        var nameEl = document.getElementById('contractName');
        var partyASelect = document.getElementById('contractPartyA');
        var partyBSelect = document.getElementById('contractPartyB');
        var amountEl = document.getElementById('contractAmount');

        var type = typeSelect ? typeSelect.value : '';
        var name = nameEl ? nameEl.value : '';
        var partyA = partyASelect ? partyASelect.value : '';
        var partyB = partyBSelect ? partyBSelect.value : '';

        if (!type) {
            showAppModal('提示', '请选择合同类型', null, false);
            return;
        }
        if (!name.trim()) {
            showAppModal('提示', '请输入合同名称', null, false);
            return;
        }
        if (!partyA) {
            showAppModal('提示', '请选择甲方', null, false);
            return;
        }
        if (!partyB) {
            showAppModal('提示', '请选择乙方', null, false);
            return;
        }

        showAppModal('创建确认', '确定要为"' + currentContractTarget + '"创建合同"' + name + '"吗？', function() {
            var contractId = 'contract-new-' + Date.now();
    ...
    function showExitProjectModal() {
        var modal = document.getElementById('exitProjectModal');
        if (modal) modal.classList.add('show');
    }

    function closeExitProjectModal() {
        var modal = document.getElementById('exitProjectModal');
        if (modal) modal.classList.remove('show');
    }
    ...
    window.toggleNoActivity = toggleNoActivity;
    window.toggleNoArchitecture = toggleNoArchitecture;
    window.showInviteMemberModal = showInviteMemberModal;
    window.closeInviteMemberModal = closeInviteMemberModal;
    window.selectInviteMethod = selectInviteMethod;
    window.copyInviteLink = copyInviteLink;
    window.sendInvite = sendInvite;
    window.showCreateContractModal = showCreateContractModal;
    window.closeCreateContractModal = closeCreateContractModal;
    window.createContract = createContract;
    window.showExitProjectModal = showExitProjectModal;
    window.closeExitProjectModal = closeExitProjectModal;
    window.confirmExitProject = confirmExitProject;
